...
describe('Raum-Sync-Lebenszyklus', () => {
  it('verwendet den direkt nach create laufenden Sync ohne Storage-Neustart weiter', async () => {
    ...
    expect(store.resume(host.code)).toBe(true)
  ...
  it('fordert nach einem echten Detach wieder eine gespeicherte Membership an', () => {
    const store = useRoomStore()
    store.attach(host)
    store.detach()

    expect(store.resume(host.code)).toBe(false)
    expect(roomMocks.getMembership).toHaveBeenCalledWith(host.code)
  })
